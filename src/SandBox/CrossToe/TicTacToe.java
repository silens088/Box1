...
class TicTacToe {
    ...
    TicTacToe() {
        //используем конструктор для инициализации полей
        random = new Random();
    ...
    void initTable() {
        /*
        обеспечивает начальную инициализацию игровой таблицы, заполняя её ячейки «пустыми» символами.
        Внешний цикл, со счетчиком int row, выбирает строки,
        а внутренний, со счётчиком int col, перебирает ячейки в каждой строке.
         */
    ...
    void printTable() {
        //метод, отображающий текущее состояние игровой таблицы printTable()
        for (int row = 0; row < 3; row++) {
            for (int col = 0; col < 3; col++)
